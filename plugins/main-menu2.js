...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  let tag = `https://wa.me/6283823712304`
  ...
  let str = `*Hai Ngab*`
  let isi = `😘' AsbBotzz Adalah Bot WhatsApp Bisa Membuat Stiker Dll\nKalau Ada Yg Error Bisa Langsung Report Ke Owner.\n${tag} (Owner)`
  let wibu = `https://api-reysekha.herokuapp.com/api/random/cosplay?apikey=apirey`
  let thumb = await (await fetch(wibu)).buffer()
  conn.send3ButtonImg(m.chat, `https://telegra.ph/file/6411cc04828f3cc335324.jpg`, str, isi, 'Menu', '.command', 'Owner', '.owner', m, {
    contextInfo: {
      externalAdReply: {
