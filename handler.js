                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            conn.sendButtonImg(id, await (await fetch(action === 'add' ? wel : lea)).buffer(), text, wm, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Wellcome', fkontak, {
                                contextInfo: {
                                    externalAdReply: {
...
global.dfail = async (type, m, conn) => {
    ...
    if (msgg) return conn.sendButtondDoc(m.chat, `${global.htki} VERIFY ${global.htka}`, msgg, '𝑽𝒆𝒓𝒊𝒇𝒚', '.verify', m, {
        contextInfo: {
            externalAdReply: {
                showAdAttribution: true,
